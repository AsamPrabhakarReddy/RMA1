  ...
  const handleLeaseSubmit = async (values) => {
    try {
      const res = await axios.post(
        `https://rma1-backend-9a0v.onrender.com/api/v1/addTenantLeaseAgreement?landlordLeaseAgreementId=${landlordLeaseAgreementID}&propertyId=${propertyID}&tenantId=${customerID}`,
        values,
        {
          headers: {
      ...
      if (res.status === 201) {
        ...
        const [tenantRes, landlordRes] = await Promise.all([
          axios.post(
            `https://rma1-backend-9a0v.onrender.com/api/v1/updateTenantDetailsInLandlordDashboard`,
          ...
          axios.post(
            `https://rma1-backend-9a0v.onrender.com/api/v1/getLandlordDetailsInTenantDashboard`,
